...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val neighbours = for {
        b <- initial.toSet[(Block, List[Move])]
        n <- newNeighborsOnly(neighborsWithHistory(b._1, b._2), explored).toSet[(Block, List[Move])]
      } yield n
      initial ++ from(neighbours.toStream, explored ++ (initial map (_._1)).toSet)
    }

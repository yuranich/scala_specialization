...
class FunSetSuite extends FunSuite {
  ...
  test("test filter function") {
    new TestSets {
      ...
      val filtered = filter(s, (x: Int) => x != 2)
  ...
  test("test forall function") {
    new TestSets {
      val s = union(union(s1, s2), s3)
      assert(forall(s, (x: Int) => x < 10))
      assert(!forall(s, (x: Int) => x < 3))
      assert(forall(s, (x: Int) => x > 0))
      assert(!forall(s, (x: Int) => x > 1))
    }
  }

  test("test exists function") {
    new TestSets {
      val s = union(union(s1, s3), s1)
      assert(exists(s, (x: Int) => x == 3))
      assert(!exists(s, (x: Int) => x > 10))
      assert(exists(s, (x: Int) => x > 0))
  ...
  test("test map function") {
    new TestSets {
      val s = union(union(s1, s2), s3)
      val mapped = map(s, (x: Int) => x * 5)
      assert(contains(mapped, 5))
